...
function lookupEmail(userId: string) {
  // replace with prisma query
  return `${userId}@example.com`;
}

async function findCustomerByMetadata(userId: string) {
  const list = await stripe.customers.list({ limit: 1, email: lookupEmail(userId) });
  return list.data[0].id;
}

// verify webhook
export function verifyStripe(sig: string | string[] | undefined, raw: Buffer) {
  return stripe.webhooks.constructEvent(raw, sig as string, process.env.STRIPE_WEBHOOK_SECRET!);
}
