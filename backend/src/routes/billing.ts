...
r.post('/checkout', requireAuth, async (req, res) => {
  const sess = await bill.createCheckout((req as any).userId);
  res.json({ url: sess.url });
});

r.post('/portal', requireAuth, async (req, res) => {
  const sess = await bill.portal((req as any).userId);
  res.json({ url: sess.url });
});
...
r.post(
  ...
  (req, res) => {
    try {
      ...
      res.status(400).send(`Webhook Error: ${(e as any).message}`);
    }
  }
);
